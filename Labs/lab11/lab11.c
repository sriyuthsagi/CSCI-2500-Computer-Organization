...
uint32_t get_index( uint32_t address, uint32_t idx_start, uint32_t idx_end )
{
  ...
  /* YOUR CODE HERE */
    int sum = 0;
    // Right shifts until the rightmost value is least significant bit
    for(int i = 0; i < idx_start; i++){
        address = address >> 1;
    }

    int shiftdist = idx_end - idx_start + 1;
...
uint32_t get_cache_block_size( uint32_t idx_start, uint32_t idx_end, uint32_t bob )
{
  ...
  /* YOUR CODE HERE */
    ...
    uint32_t bob_size = bob + 1;
    bob_size = (1 << bob_size) * 8 + tag_size;
